...
describe('DispatcherMappings.vue', () => {
  describe('dispatcherServlet available', () => {
    ...
    it('should render produces information', async () => {
      const produces = screen.getByRole('cell', { name: 'application/vnd.spring-boot.actuator.v3+json, application/vnd.spring-boot.actuator.v2+json, application/json' })
      expect(produces).toBeVisible()
    })

    it('should render http-verbs', async () => {
      const httpVerbs = screen.getByRole('cell', { name: 'GET, POST, PUT, DELETE, OPTIONS' })
