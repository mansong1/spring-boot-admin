...
class AdminApplicationDiscoveryTest {
  ...
  void setUp() {
    this.instance =
        new SpringApplicationBuilder()
            .sources(TestAdminApplication.class)
  ...
  private URI registerInstance() {
    ...
    return URI.create(
        "http://localhost:" + this.port + "/instances/" + applications.get(0).optString("id"));
  }
  ...
  private void listInstances() {
    ...
    this.webClient
        ...
        .isOk()
        .expectBody()
        .jsonPath("$[0].registration.name")
        .isEqualTo("Test-Instance")
        .jsonPath("$[0].statusInfo.status")
        .isEqualTo("UP")
        .jsonPath("$[0].info.test")
        .isEqualTo("foobar");
    // @formatter:on
  }
