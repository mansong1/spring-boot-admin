...
public class InstanceDiscoveryListenerTest {
    ...
    when(this.discovery.getInstances("service"))
        .thenReturn(
            Collections.singletonList(
                new DefaultServiceInstance("test-1", "service", "localhost", 80, false)));
  ...
  public void should_not_register_instance_when_serviceId_is_ignored() {
    ...
    when(this.discovery.getInstances("service"))
        .thenReturn(
            singletonList(new DefaultServiceInstance("test-1", "service", "localhost", 80, false)));
        ...
        .thenReturn(
            singletonList(
                new DefaultServiceInstance(
                    ...
                    80,
                    false,
                    Collections.singletonMap("monitoring", "true"))));
  ...
  @Test
  public void
      should_register_instances_when_serviceId_matches_wanted_pattern_and_ignored_pattern() {
    when(this.discovery.getServices())
        .thenReturn(asList("service-1", "service", "rabbit-1", "rabbit-2"));
    when(this.discovery.getInstances("service"))
        .thenReturn(
            singletonList(new DefaultServiceInstance("test-1", "service", "localhost", 80, false)));
    when(this.discovery.getInstances("service-1"))
        .thenReturn(
            singletonList(
                new DefaultServiceInstance("test-1", "service-1", "localhost", 80, false)));
  ...
  @Test
  public void
      should_not_register_instances_when_instanceMetadata_matches_wanted_metadata_and_ignored_metadata() {
    ...
    StepVerifier.create(this.registry.getInstances())
        .assertNext(
            (application) -> {
              ...
              assertThat(registration.getHealthUrl())
                  .isEqualTo("http://localhost:80/actuator/health");
  ...
  public void should_remove_instances_when_they_are_no_longer_available_in_discovery() {
    StepVerifier.create(
            this.registry.register(Registration.create("ignored", "http://health").build()))
        .consumeNextWith((id) -> {})
        .verifyComplete();
    StepVerifier.create(
            this.registry.register(
                Registration.create("different-source", "http://health2")
                    .source("http-api")
                    .build()))
        .consumeNextWith((id) -> {})
        .verifyComplete();
