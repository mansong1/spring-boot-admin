...
public class RegistrationApplicationListenerTest {
  ...
  public void should_schedule_register_task() {
    ...
    RegistrationApplicationListener listener =
        new RegistrationApplicationListener(registrator, scheduler);

    listener.onApplicationReady(
        new ApplicationReadyEvent(
            mock(SpringApplication.class), null, mock(ConfigurableWebApplicationContext.class)));
  ...
  public void should_cancel_register_task_on_context_close() {
    ...
    when(scheduler.scheduleAtFixedRate(isA(Runnable.class), eq(Duration.ofSeconds(10))))
        .then((invocation) -> task);
