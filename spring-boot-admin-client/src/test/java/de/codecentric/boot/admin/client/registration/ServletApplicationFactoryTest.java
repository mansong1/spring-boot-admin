...
public class ServletApplicationFactoryTest {
  ...
  private ServletApplicationFactory factory =
      new ServletApplicationFactory(
          instance,
          management,
          server,
          servletContext,
  ...
  public void test_servletPath() {
    ...
    Application app = factory.createApplication();
    assertThat(app.getManagementUrl())
        .isEqualTo("http://" + getHostname() + ":80/srv/app/actuator");
    assertThat(app.getHealthUrl())
        .isEqualTo("http://" + getHostname() + ":80/srv/app/actuator/health");
  ...
  public void test_servicePath() {
    ...
    Application app = factory.createApplication();
    assertThat(app.getManagementUrl())
        .isEqualTo("http://" + getHostname() + ":80/servicePath/app/actuator");
    assertThat(app.getHealthUrl())
        .isEqualTo("http://" + getHostname() + ":80/servicePath/app/actuator/health");
  ...
  private void publishApplicationReadyEvent(
      DefaultApplicationFactory factory, Integer serverport, Integer managementport) {
    ...
    factory.onWebServerInitialized(
        new TestWebServerInitializedEvent(
            "management", (managementport != null) ? managementport : serverport));
  }
