...
public class SpringBootAdminClientAutoConfiguration {
  ...
  public ApplicationRegistrator registrator(
    ...
    return new DefaultApplicationRegistrator(
        applicationFactory, registrationClient, client.getAdminUrl(), client.isRegisterOnce());
  }

  @Bean
  @ConditionalOnMissingBean
  public RegistrationApplicationListener registrationListener(
      ClientProperties client, ApplicationRegistrator registrator, Environment environment) {
  ...
  public static class ServletConfiguration {
    ...
    public ApplicationFactory applicationFactory(
      ...
      return new ServletApplicationFactory(
          instance,
          management,
          server,
          servletContext,
          pathMappedEndpoints,
          webEndpoint,
          new CompositeMetadataContributor(
              metadataContributors.getIfAvailable(Collections::emptyList)),
  ...
  public static class ReactiveRegistrationClientConfig {
    ...
    @Bean
    @ConditionalOnMissingBean
    public ReactiveRegistrationClient registrationClient(
        ClientProperties client, WebClient.Builder webClient) {
      if (client.getUsername() != null && client.getPassword() != null) {
        webClient =
            webClient.filter(basicAuthentication(client.getUsername(), client.getPassword()));
      }
