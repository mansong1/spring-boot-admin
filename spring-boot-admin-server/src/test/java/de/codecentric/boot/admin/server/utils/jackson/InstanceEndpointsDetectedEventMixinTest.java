...
public class InstanceEndpointsDetectedEventMixinTest {
  ...
  public void verifyDeserialize() throws JSONException, JsonProcessingException {
    String json =
        new JSONObject()
            ...
            .put(
                ...
                new JSONArray()
                    .put(
                        new JSONObject().put("id", "info").put("url", "http://localhost:8080/info"))
                    .put(
                        new JSONObject()
                            .put("id", "health")
                            .put("url", "http://localhost:8080/health")))
            .toString();

    InstanceEndpointsDetectedEvent event =
        objectMapper.readValue(json, InstanceEndpointsDetectedEvent.class);
    ...
    assertThat(event.getEndpoints())
        .containsExactlyInAnyOrder(
            Endpoint.of("info", "http://localhost:8080/info"),
  ...
  @Test
  public void verifyDeserializeWithOnlyRequiredProperties()
  ...
  public void verifySerialize() throws IOException {
    ...
    Endpoints endpoints =
        Endpoints.single("info", "http://localhost:8080/info")
            .withEndpoint("health", "http://localhost:8080/health");
    InstanceEndpointsDetectedEvent event =
        new InstanceEndpointsDetectedEvent(id, 12345678L, timestamp, endpoints);
    ...
    assertThat(jsonContent)
        .extractingJsonPathNumberValue("$.timestamp")
        .isEqualTo(1587751031.000000000);
    ...
    assertThat(jsonContent)
        .extractingJsonPathStringValue("$.endpoints[0].id")
        .isIn("info", "health");
    assertThat(jsonContent)
        .extractingJsonPathStringValue("$.endpoints[0].url")
        .isIn("http://localhost:8080/info", "http://localhost:8080/health");

    assertThat(jsonContent)
        .extractingJsonPathStringValue("$.endpoints[1].id")
        .isIn("info", "health");
    assertThat(jsonContent)
        .extractingJsonPathStringValue("$.endpoints[1].url")
        .isIn("http://localhost:8080/info", "http://localhost:8080/health");
  }
  ...
  public void verifySerializeWithOnlyRequiredProperties() throws IOException {
    ...
    Instant timestamp = Instant.ofEpochSecond(1587751031).truncatedTo(ChronoUnit.SECONDS);
    InstanceEndpointsDetectedEvent event =
        new InstanceEndpointsDetectedEvent(id, 0L, timestamp, null);
  ...
  public void verifySerializeWithEmptyEndpoints() throws IOException {
    ...
    Instant timestamp = Instant.ofEpochSecond(1587751031).truncatedTo(ChronoUnit.SECONDS);
    InstanceEndpointsDetectedEvent event =
        new InstanceEndpointsDetectedEvent(id, 0L, timestamp, Endpoints.empty());
