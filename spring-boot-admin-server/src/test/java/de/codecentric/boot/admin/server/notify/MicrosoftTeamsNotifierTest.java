...
class MicrosoftTeamsNotifierTest {
  ...
  void setUp() {
    instance =
        Instance.create(InstanceId.of(appId))
            .register(
                Registration.create(appName, healthUrl)
  ...
  void test_onClientApplicationDeRegisteredEvent_resolve() {
    ...
    verify(mockRestTemplate)
        .postForEntity(eq(URI.create("http://example.com")), entity.capture(), eq(Void.class));

    assertThat(entity.getValue().getHeaders().getContentType())
        .isEqualTo(MediaType.APPLICATION_JSON);
    assertMessage(
        entity.getValue().getBody(),
  ...
  void test_onApplicationRegisteredEvent_resolve() {
    InstanceRegisteredEvent event =
        new InstanceRegisteredEvent(instance.getId(), 1L, instance.getRegistration());
  ...
  void test_onApplicationStatusChangedEvent_resolve() {
    InstanceStatusChangedEvent event =
        new InstanceStatusChangedEvent(instance.getId(), 1L, StatusInfo.ofUp());
  ...
  void test_getDeregisteredMessageForAppReturns_correctContent() {
    Message message =
        notifier.getDeregisteredMessage(
            instance,
            notifier.createEvaluationContext(
                new InstanceDeregisteredEvent(instance.getId(), 1L), instance));
  ...
  void test_getStatusChangedMessageForAppReturns_correctContent() {
    Message message =
        notifier.getStatusChangedMessage(
            instance,
            notifier.createEvaluationContext(
                new InstanceStatusChangedEvent(instance.getId(), 1L, StatusInfo.ofDown()),
                instance));
  ...
  @Test
  void
      test_getStatusChangedMessageWithExtraFormatArgumentReturns_activitySubtitlePatternWithAppName() {
    ...
    assertThat(message.getSections().get(0).getActivitySubtitle())
        .isEqualTo("STATUS_ACTIVITY_PATTERN_" + appName);
  }
  ...
  @Test
  void
      test_getDeRegisterMessageWithExtraFormatArgumentReturns_activitySubtitlePatternWithAppName() {
    notifier.setDeregisterActivitySubtitle(
        "DEREGISTER_ACTIVITY_PATTERN_#{instance.registration.name}");
    Message message =
        notifier.getDeregisteredMessage(
            instance,
            notifier.createEvaluationContext(
  ...
  void test_getStatusChangedMessage_parsesThemeColorFromSpelExpression() {
    notifier.setThemeColor(
        "#{event.type == 'STATUS_CHANGED' ? (event.statusInfo.status=='UP' ? 'green' : 'red') :"
            + " 'blue'}");
  ...
  private void assertMessage(
    ...
    assertThat(message.getSections())
        .hasSize(1)
        .anySatisfy(
            (section) -> {
              assertThat(section.getActivityTitle())
                  .isEqualTo(instance.getRegistration().getName());
              assertThat(section.getActivitySubtitle()).isEqualTo(expectedSubTitle);

              assertThat(section.getFacts())
                  .hasSize(5)
                  .anySatisfy(
                      (fact) -> {
