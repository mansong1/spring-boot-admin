...
public class BasicAuthHttpHeaderProviderTest {
  ...
  private final BasicAuthHttpHeaderProvider headersProviderEnableInstanceAuth =
      new BasicAuthHttpHeaderProvider(
          "client",
          "client",
          Collections.singletonMap("sb-admin-server", new InstanceCredentials("admin", "admin")));

  @Test
  public void test_auth_header() {
    Registration registration =
        Registration.create("foo", "http://health")
  ...
  public void test_auth_instance_enabled_use_default_creds() {
    Registration registration =
        Registration.create("foo", "http://health").name("xyz-server").build();
    Instance instance = Instance.create(InstanceId.of("id")).register(registration);
    assertThat(
            this.headersProviderEnableInstanceAuth
                .getHeaders(instance)
  ...
  public void test_auth_instance_enabled_use_service_specific_creds() {
    Registration registration =
        Registration.create("foo", "http://health").name("sb-admin-server").build();
    Instance instance = Instance.create(InstanceId.of("id")).register(registration);
    assertThat(
            this.headersProviderEnableInstanceAuth
                .getHeaders(instance)
