...
public class SnapshottingInstanceRepositoryTest extends AbstractInstanceRepositoryTest {
  ...
  public void should_return_instance_from_cache() {
    // given
    StepVerifier.create(this.repository.save(this.instance))
        .expectNext(this.instance)
        .verifyComplete();
    // when
        ...
        .thenReturn(
            Flux.just(
                new InstanceRegisteredEvent(
                    InstanceId.of("broken"), 0L, this.instance.getRegistration()),
                new InstanceRegisteredEvent(
                    InstanceId.of("broken"), 0L, this.instance.getRegistration()),
                new InstanceRegisteredEvent(
                    this.instance.getId(), 0L, this.instance.getRegistration()),
                new InstanceRegisteredEvent(
                    InstanceId.of("broken"), 1L, this.instance.getRegistration())));
    // when
    this.repository.start();
    // then
    reset(this.eventStore);
  ...
  public void should_return_outdated_instance_not_present_in_cache() {
    ...
    StepVerifier.create(this.repository.save(this.instance))
        ...
        .verifyComplete();
    StepVerifier.create(this.repository.save(this.instance))
        .verifyError(OptimisticLockingException.class);
    // when
    StepVerifier.create(this.repository.find(this.instance.getId()))
        .expectNext(this.instance)
        .verifyComplete();
  ...
  public void should_refresh_snapshots_eagerly_on_optimistick_lock_exception() {
    ...
    StepVerifier.create(
            this.repository.save(
                this.instance.clearUnsavedEvents().withStatusInfo(StatusInfo.ofDown())))
        .expectNextCount(1L)
        .verifyComplete();
    // when
    StepVerifier.create(
            this.repository.computeIfPresent(
                this.instance.getId(), (id, i) -> Mono.just(i.withStatusInfo(StatusInfo.ofUp()))))
        .expectNextCount(1L)
        .verifyComplete();
  }
}
