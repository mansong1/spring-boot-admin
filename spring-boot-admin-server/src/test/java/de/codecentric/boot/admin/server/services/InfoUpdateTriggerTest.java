...
public class InfoUpdateTriggerTest {
  ...
  public void should_update_on_endpoints_detectes_event() {
    ...
    this.events.next(
        new InstanceEndpointsDetectedEvent(
            this.instance.getId(), this.instance.getVersion(), this.instance.getEndpoints()));
    // then should update
  ...
  public void should_not_update_on_non_relevant_event() {
    ...
    this.events.next(
        new InstanceInfoChangedEvent(
            this.instance.getId(), this.instance.getVersion(), Info.empty()));
    // then should not update
    verify(this.updater, never()).updateInfo(this.instance.getId());
  }
  ...
  public void should_continue_update_after_error() throws InterruptedException {
    ...
    when(this.updater.updateInfo(any()))
    ...
    this.events.next(
        new InstanceStatusChangedEvent(
            this.instance.getId(), this.instance.getVersion(), StatusInfo.ofUp()));
