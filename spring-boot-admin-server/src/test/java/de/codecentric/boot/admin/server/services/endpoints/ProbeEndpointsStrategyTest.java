...
public class ProbeEndpointsStrategyTest {
  ...
  public void invariants() {
    ...
    assertThatThrownBy(
            () -> new ProbeEndpointsStrategy(this.instanceWebClient, new String[] {null}))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  public void should_return_detect_endpoints() {
    ...
    this.wireMock.stubFor(
        options(urlEqualTo("/mgmt/exception"))
            .willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));

    ProbeEndpointsStrategy strategy =
    ...
    StepVerifier.create(strategy.detectEndpoints(instance))
        // then
        .expectNext(
            Endpoints.single("metrics", this.wireMock.url("/mgmt/stats"))
  ...
  public void should_return_empty() {
    ...
    this.wireMock.stubFor(
        options(urlEqualTo("/mgmt/stats"))
            .willReturn(aResponse().withStatus(HttpStatus.NOT_FOUND_404)));

    ProbeEndpointsStrategy strategy =
        new ProbeEndpointsStrategy(this.instanceWebClient, new String[] {"metrics:stats"});
  ...
  public void should_retry() {
    ...
    this.wireMock.stubFor(
        options(urlEqualTo("/mgmt/metrics"))
            .inScenario("retry")
    ...
    ProbeEndpointsStrategy strategy =
        new ProbeEndpointsStrategy(
            this.instanceWebClient, new String[] {"metrics:stats", "metrics", "info", "non-exist"});
