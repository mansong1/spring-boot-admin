...
class InstanceExchangeFilterFunctionsTest {
  ...
  class ConvertLegacyEndpoints {
    ...
    private final DataBuffer original =
        this.bufferFactory.wrap("ORIGINAL".getBytes(StandardCharsets.UTF_8));

    private final DataBuffer converted =
        this.bufferFactory.wrap("CONVERTED".getBytes(StandardCharsets.UTF_8));

    private final InstanceExchangeFilterFunction filter =
        InstanceExchangeFilterFunctions.convertLegacyEndpoints(
            singletonList(
    ...
    void should_convert_v1_actuator() {
      ...
      Mono<ClientResponse> response =
          this.filter.filter(INSTANCE, request, (r) -> Mono.just(legacyResponse));

      StepVerifier.create(response)
          .assertNext(
              (r) -> {
                assertThat(r.headers().contentType())
                    .hasValue(MediaType.valueOf(ActuatorMediaType.V2_JSON));
                assertThat(r.headers().contentLength()).isEmpty();
                StepVerifier.create(r.body(BodyExtractors.toDataBuffers()))
                    .expectNext(this.converted)
                    .verifyComplete();
              })
          .verifyComplete();
    }
    ...
    void should_not_convert_v2_actuator() {
      ...
      Mono<ClientResponse> convertedResponse =
          filter.filter(INSTANCE, request, (r) -> Mono.just(response));
    ...
    void should_not_convert_without_converter() {
      ...
      ClientRequest request =
          ClientRequest.create(HttpMethod.GET, URI.create("/unknown"))
  ...
  class Retry {
    ...
    void should_retry_using_default() {
      ...
      StepVerifier.create(filter.filter(INSTANCE, request, exchange))
          .expectNext(response)
          .verifyComplete();
    ...
    void should_retry_using_endpoint_value_default() {
      InstanceExchangeFilterFunction filter =
          InstanceExchangeFilterFunctions.retry(0, singletonMap("test", 1));
    ...
    void should_not_retry_for_put_post_patch_delete() {
      ...
      ClientRequest patchRequest =
          ClientRequest.create(HttpMethod.PATCH, URI.create("/test")).build();
      ...
      StepVerifier.create(filter.filter(INSTANCE, putRequest, exchange))
          .verifyError(IllegalStateException.class);
      assertThat(invocationCount.get()).isEqualTo(1);

      invocationCount.set(0L);
      ClientRequest postRequest =
          ClientRequest.create(HttpMethod.POST, URI.create("/test")).build();
      ...
      ClientRequest deleteRequest =
          ClientRequest.create(HttpMethod.DELETE, URI.create("/test")).build();
  ...
  class AddHeaders {
    ...
    private InstanceExchangeFilterFunction filter =
        InstanceExchangeFilterFunctions.addHeaders(
            (i) -> {
    ...
    void should_add_headers_from_provider() {
      ...
      Mono<ClientResponse> response =
          this.filter.filter(
              ...
              request,
              (req) -> {
                assertThat(req.headers().get("X-INSTANCE-ID"))
                    .containsExactly(INSTANCE.getId().getValue());
    ...
    void should_add_default_accept_headers() {
      ...
      Mono<ClientResponse> response =
          this.filter.filter(
              ...
              request,
              (req) -> {
                assertThat(req.headers().getAccept())
                    .containsExactly(
                        ACTUATOR_V2_MEDIATYPE, ACTUATOR_V1_MEDIATYPE, MediaType.APPLICATION_JSON);
                return Mono.just(ClientResponse.create(HttpStatus.OK).build());
              });
  ...
  class RewriteEndpointUrl {
    ...
    private final Registration registration =
        Registration.create("R", "http://test/actuator/health")
    ...
    private final Instance instance =
        Instance.create(InstanceId.of("R"))
    ...
    void should_rewirte_url_and_add_endpoint_attribute() {
      ClientRequest request =
          ClientRequest.create(HttpMethod.GET, URI.create("health/database"))
      ...
      Mono<ClientResponse> response =
          this.filter.filter(
              ...
              (req) -> {
                assertThat(req.url())
                    .isEqualTo(URI.create(this.registration.getHealthUrl() + "/database"));
    ...
    void should_not_rewrite_absolute_url() {
      ClientRequest request =
          ClientRequest.create(HttpMethod.GET, URI.create("http://test/actuator/unknown"))
              .attribute(ATTRIBUTE_INSTANCE, this.instance)
              .build();
    ...
    void should_set_endpoint_attribute_for_management_url() {
      ClientRequest request =
          ClientRequest.create(HttpMethod.GET, URI.create("http://test/actuator"))
              .attribute(ATTRIBUTE_INSTANCE, this.instance)
              .build();
    ...
    void should_error_on_unspecified_endpoint() {
      ClientRequest request =
          ClientRequest.create(HttpMethod.GET, URI.create(""))
              .attribute(ATTRIBUTE_INSTANCE, this.instance)
              .build();
      ...
      StepVerifier.create(response)
          .verifyErrorSatisfies(
              (e) ->
                  assertThat(e)
                      .isInstanceOf(ResolveEndpointException.class)
  ...
  class Timeout {
    ...
    void should_timeout_using_default() {
      InstanceExchangeFilterFunction filter =
          InstanceExchangeFilterFunctions.timeout(Duration.ofSeconds(1), emptyMap());
    ...
    void should_timeout_using_endpoint_value_default() {
      InstanceExchangeFilterFunction filter =
          InstanceExchangeFilterFunctions.timeout(
              Duration.ofSeconds(10), singletonMap("test", Duration.ofSeconds(1)));
    ...
    void should_add_accept_all_to_headers_for_logfile() {
      ...
      Mono<ClientResponse> response =
          this.filter.filter(
              ...
              (req) -> {
                assertThat(req.headers().getAccept())
                    .containsExactly(MediaType.TEXT_PLAIN, MediaType.ALL);
                return Mono.just(ClientResponse.create(HttpStatus.OK).build());
              });
  ...
  public class CookieHandling {
    ...
    void should_store_retrieved_cookie() {
      ClientRequest request =
          ClientRequest.create(HttpMethod.GET, URI.create("http://localhost/test")).build();
      ...
      Mono<ClientResponse> response =
          this.filter.filter(
              ...
              (req) ->
                  Mono.just(
                      ClientResponse.create(HttpStatus.OK)
                          .header("Set-Cookie", "testCookie=testCookieValue")
                          .build()));
      ...
      assertThat(captor.getValue())
          .containsEntry("Set-Cookie", singletonList("testCookie=testCookieValue"));
    }
    ...
    void should_add_stored_cookie_to_request() {
      ...
      when(this.cookieStore.get(eq(INSTANCE.getId()), eq(request.url()), any()))
