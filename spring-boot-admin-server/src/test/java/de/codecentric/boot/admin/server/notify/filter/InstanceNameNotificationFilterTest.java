...
public class InstanceNameNotificationFilterTest {
  ...
  public void test_expiry() throws InterruptedException {
    ...
    ExpiringNotificationFilter filterExpired =
        new ApplicationNameNotificationFilter("foo", Instant.now().minus(Duration.ofSeconds(1)));
    ExpiringNotificationFilter filterLong =
        new ApplicationNameNotificationFilter("foo", Instant.now().plus(Duration.ofMillis(100)));
