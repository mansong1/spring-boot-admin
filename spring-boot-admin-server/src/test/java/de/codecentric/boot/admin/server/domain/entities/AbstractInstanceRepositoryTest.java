...
public abstract class AbstractInstanceRepositoryTest {
  ...
  private final Instance instance1 =
      Instance.create(InstanceId.of("app-1"))
          .register(Registration.create("app", "http://health").build());

  private final Instance instance2 =
      Instance.create(InstanceId.of("app-2"))
          .register(Registration.create("app", "http://health").build());

  private final Instance instance3 =
      Instance.create(InstanceId.of("other-1"))
  ...
  public void should_save() {
    // when
    StepVerifier.create(this.repository.save(this.instance1))
        .expectNext(this.instance1)
        .verifyComplete();
    // then
  ...
  public void should_find_instances() {
    ...
    // when/then
    StepVerifier.create(this.repository.find(this.instance2.getId()))
        .expectNext(this.instance2)
        .verifyComplete();
    ...
    StepVerifier.create(this.repository.findAll().collectList())
        .assertNext(
            (v) ->
                assertThat(v)
                    .containsExactlyInAnyOrder(this.instance1, this.instance2, this.instance3))
        .verifyComplete();
  }
  ...
  public void should_not_compute_if_not_present() {
    ...
    StepVerifier.create(
            this.repository.computeIfPresent(
                instanceId,
                (key, application) ->
                    Mono.error(new AssertionFailedError("Should not call any computation"))))
        .verifyComplete();
  ...
  public void should_run_compute_with_null() {
    ...
    // then
    StepVerifier.create(this.repository.find(instanceId))
        .assertNext(
            (v) -> {
              assertThat(v.getId()).isEqualTo(instanceId);
              assertThat(v.getRegistration()).isEqualTo(registration);
