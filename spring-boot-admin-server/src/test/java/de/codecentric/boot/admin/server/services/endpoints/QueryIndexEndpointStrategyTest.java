...
public class QueryIndexEndpointStrategyTest {
  ...
  public void should_return_endpoints() {
    ...
    Instance instance =
        Instance.create(InstanceId.of("id"))
            .register(
                Registration.create("test", this.wireMock.url("/mgmt/health"))
                    .managementUrl(this.wireMock.url("/mgmt"))
                    .build());

    String host = "https://localhost:" + this.wireMock.httpsPort();
    String body =
        "{\"_links\":{\"metrics-requiredMetricName\":{\"templated\":true,\"href\":\""
            + host
    ...
    this.wireMock.stubFor(
        get("/mgmt").willReturn(ok(body).withHeader("Content-Type", ActuatorMediaType.V2_JSON)));
    ...
    StepVerifier.create(strategy.detectEndpoints(instance))
        // then
        .expectNext(
            Endpoints.single("metrics", host + "/mgmt/stats")
  ...
  public void should_return_endpoints_with_aligned_scheme() {
    ...
    StepVerifier.create(strategy.detectEndpoints(instance))
        // then
        .expectNext(
            Endpoints.single("metrics", secureHost + "/mgmt/stats")
  ...
  public void should_return_empty_on_empty_endpoints() {
    ...
    this.wireMock.stubFor(
        get("/mgmt")
            .willReturn(okJson(body).withHeader("Content-Type", ActuatorMediaType.V2_JSON)));
  ...
  public void should_return_empty_on_wrong_content_type() {
    ...
    this.wireMock.stubFor(
        get("/mgmt").willReturn(ok(body).withHeader("Content-Type", MediaType.TEXT_PLAIN_VALUE)));
  ...
  public void should_return_empty_when_mgmt_equals_service_url() {
    ...
    Instance instance =
        Instance.create(InstanceId.of("id"))
            .register(
                Registration.create("test", this.wireMock.url("/app/health"))
  ...
  public void should_retry() {
    ...
    String body =
        "{\"_links\":{\"metrics-requiredMetricName\":{\"templated\":true,\"href\":\"/mgmt/metrics/{requiredMetricName}\"},\"self\":{\"templated\":false,\"href\":\"/mgmt\"},\"metrics\":{\"templated\":false,\"href\":\"/mgmt/stats\"},\"info\":{\"templated\":false,\"href\":\"/mgmt/info\"}}}";
    ...
    StepVerifier.create(strategy.detectEndpoints(instance))
        // then
        .expectNext(
            Endpoints.single("metrics", "/mgmt/stats").withEndpoint("info", "/mgmt/info")) //
        .verifyComplete();
  }
