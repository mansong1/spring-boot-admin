...
public class StatusUpdaterTest {
  ...
  public void should_change_status_to_down() {
    ...
    this.wireMock.stubFor(
        get("/health")
            .willReturn(
                okForContentType(ActuatorMediaType.V2_JSON, body)
    ...
    StepVerifier.create(this.eventStore)
        ...
        .then(
            () ->
                StepVerifier.create(this.updater.updateStatus(this.instance.getId()))
                    .verifyComplete())
        .assertNext(
            (event) -> {
              ...
              assertThat(statusChangedEvent.getStatusInfo().getDetails())
                  .isEqualTo(singletonMap("foo", "bar"));
            })
        .thenCancel()
        .verify();
    ...
    StepVerifier.create(
            this.repository.computeIfPresent(
                this.instance.getId(), (key, instance) -> Mono.just(instance.deregister())))
        .then(
            () ->
                StepVerifier.create(this.updater.updateStatus(this.instance.getId()))
                    .verifyComplete())
  ...
  public void should_not_change_status() {
    ...
    this.wireMock.stubFor(
        get("/health")
            .willReturn(okJson(body).withHeader("Content-Type", Integer.toString(body.length()))));
  ...
  public void should_change_status_to_down_without_details_incompatible_content_type() {
    ...
    StepVerifier.create(this.repository.find(this.instance.getId()))
        .assertNext(
            (app) -> {
              assertThat(app.getStatusInfo().getStatus()).isEqualTo("DOWN");
              assertThat(app.getStatusInfo().getDetails())
  ...
  public void should_change_status_to_down_without_details_no_body() {
    this.wireMock.stubFor(
        get("/health")
            .willReturn(status(503).withHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)));
  ...
  public void should_retry() {
    ...
    this.wireMock.stubFor(
        get("/health").inScenario("retry").whenScenarioStateIs("recovered").willReturn(ok()));
