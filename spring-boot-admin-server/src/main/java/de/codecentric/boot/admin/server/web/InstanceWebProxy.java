...
/**
 * Forwards a request to a single instances endpoint and will respond with: - 502 (Bad Gateway) when
 * any error occurs during the request - 503 (Service unavailable) when the instance is not found -
...
public class InstanceWebProxy {
  ...
  public <V> Mono<V> forward(
      Mono<Instance> instanceMono,
      ForwardRequest forwardRequest,
      Function<ClientResponse, Mono<V>> responseHandler) {
    return instanceMono
        .defaultIfEmpty(NULL_INSTANCE)
        .flatMap(
            (instance) -> {
                ...
                return Mono.defer(
                    () ->
                        responseHandler.apply(
                            ClientResponse.create(HttpStatus.SERVICE_UNAVAILABLE, this.strategies)
                                .build()));
              }
            });
  }
    ...
    return instances.flatMap(
        (instance) ->
            this.forward(
                ...
                forwardRequest,
                (clientResponse) -> {
                  InstanceResponse.Builder response =
                      InstanceResponse.builder()
                          .instanceId(instance.getId())
                          .status(clientResponse.rawStatusCode())
                          .contentType(
                              String.join(
                                  ", ", clientResponse.headers().header(HttpHeaders.CONTENT_TYPE)));
  ...
  private <V> Mono<V> forward(
      Instance instance,
      ForwardRequest forwardRequest,
      Function<ClientResponse, Mono<V>> responseHandler) {
    log.trace(
        "Proxy-Request for instance {} with URL '{}'", instance.getId(), forwardRequest.getUri());
    ...
    return headersSpec
        ...
        .onErrorResume(
            ...
            (ex) -> {
              ...
              return responseHandler.apply(
                  ClientResponse.create(HttpStatus.NOT_FOUND, this.strategies).build());
            })
        .onErrorResume(
            ...
            (ex) -> {
              ...
              if (cause instanceof ReadTimeoutException || cause instanceof TimeoutException) {
                log.trace(
                    ...
                    forwardRequest.getUri());
                return responseHandler.apply(
                    ClientResponse.create(HttpStatus.GATEWAY_TIMEOUT, this.strategies).build());
              }
              if (cause instanceof IOException) {
                log.trace(
                    ...
                    cause);
                return responseHandler.apply(
                    ClientResponse.create(HttpStatus.BAD_GATEWAY, this.strategies).build());
              }
