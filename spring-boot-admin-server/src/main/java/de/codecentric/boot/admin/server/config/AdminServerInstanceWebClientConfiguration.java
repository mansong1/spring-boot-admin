...
public class AdminServerInstanceWebClientConfiguration {
  ...
  public AdminServerInstanceWebClientConfiguration(
      ObjectProvider<InstanceWebClientCustomizer> customizers, WebClient.Builder webClient) {
    this.instanceWebClientBuilder = InstanceWebClient.builder(webClient);
    customizers
        .orderedStream()
        .forEach((customizer) -> customizer.customize(this.instanceWebClientBuilder));
  }
  ...
  protected static class InstanceExchangeFiltersConfiguration {
    ...
    protected static class DefaultInstanceExchangeFiltersConfiguration {
      ...
      public InstanceExchangeFilterFunction addHeadersInstanceExchangeFilter(
        ...
        return InstanceExchangeFilterFunctions.addHeaders(
            new CompositeHttpHeadersProvider(headersProviders));
      }
      ...
      public InstanceExchangeFilterFunction retryInstanceExchangeFilter(
        ...
        return InstanceExchangeFilterFunctions.retry(
            monitor.getDefaultRetries(), monitor.getRetries());
      }
      ...
      public InstanceExchangeFilterFunction timeoutInstanceExchangeFilter(
          AdminServerProperties adminServerProperties) {
        AdminServerProperties.MonitorProperties monitor = adminServerProperties.getMonitor();
        return InstanceExchangeFilterFunctions.timeout(
            monitor.getDefaultTimeout(), monitor.getTimeout());
      }
    }
  }
  ...
  protected static class HttpHeadersProviderConfiguration {
    ...
    public BasicAuthHttpHeaderProvider basicAuthHttpHeadersProvider(
        AdminServerProperties adminServerProperties) {
      AdminServerProperties.InstanceAuthProperties instanceAuth =
          adminServerProperties.getInstanceAuth();

      if (instanceAuth.isEnabled()) {
        return new BasicAuthHttpHeaderProvider(
            instanceAuth.getDefaultUserName(),
  ...
  protected static class CookieStoreConfiguration {
    ...
    /**
     * Creates a default trigger to cleanup the cookie store on deregistering of an {@link
    ...
    @ConditionalOnMissingBean
    public CookieStoreCleanupTrigger cookieStoreCleanupTrigger(
