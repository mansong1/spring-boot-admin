...
public class RemindingNotifier extends AbstractEventNotifier {
  ...
  public void start() {
    ...
    this.subscription =
        Flux.interval(this.checkReminderInverval, this.reminderScheduler)
            ...
            .retryWhen(
                Retry.indefinitely()
                    .doBeforeRetry(
                        (s) -> log.warn("Unexpected error when sending reminders", s.failure())))
            .subscribe();
  }
  ...
  protected Mono<Void> sendReminders() {
    ...
    return Flux.fromIterable(this.reminders.values())
        .filter(
            (reminder) -> reminder.getLastNotification().plus(this.reminderPeriod).isBefore(now))
        .flatMap(
            (reminder) ->
                this.delegate
                    .notify(reminder.getEvent())
