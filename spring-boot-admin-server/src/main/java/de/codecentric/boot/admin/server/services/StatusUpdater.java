...
public class StatusUpdater {
  ...
  public Mono<Void> updateStatus(InstanceId id) {
    return this.repository
        .computeIfPresent(id, (key, instance) -> this.doUpdateStatus(instance))
        .then();
  }
  ...
  protected Mono<StatusInfo> convertStatusInfo(ClientResponse response) {
    Boolean hasCompatibleContentType =
        response
            .headers()
            .contentType()
            .map(
    ...
    StatusInfo statusInfoFromStatus =
        this.getStatusInfoFromStatus(response.statusCode(), emptyMap());
