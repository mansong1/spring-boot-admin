...
/**
 * Registry for all application instances that should be managed/administrated by the Spring Boot
 * Admin server. Backed by an InstanceRepository for persistence and an InstanceIdGenerator for id
 * generation.
 */
public class InstanceRegistry {
  ...
  public Mono<InstanceId> deregister(InstanceId id) {
    return repository
        .computeIfPresent(id, (key, instance) -> Mono.just(instance.deregister()))
        .map(Instance::getId);
  }
}
