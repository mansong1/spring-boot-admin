...
/**
 * Calls the checkFn for all instances in the given time, but not before the given retention time
 * has passed. The instances which will be checked have to be registered via `markAsChecked`.
 *
...
public class IntervalCheck {
  ...
  public void start() {
    ...
    this.subscription =
        Flux.interval(this.interval)
            .doOnSubscribe(
                (s) -> log.debug("Scheduled {}-check every {}", this.name, this.interval))
            ...
            .retryWhen(
                Retry.indefinitely()
                    .doBeforeRetry(
                        (s) -> log.warn("Unexpected error in {}-check", this.name, s.failure())))
            .subscribe();
  }
