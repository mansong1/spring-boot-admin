...
public class MicrosoftTeamsNotifier extends AbstractStatusChangeNotifier {
  ...
  private static final String DEFAULT_THEME_COLOR_EXPRESSION =
      "#{event.type == 'STATUS_CHANGED' ? (event.statusInfo.status=='UP' ? '6db33f' : 'b32d36') :"
          + " '439fe0'}";

  private static final String DEFAULT_DEREGISTER_ACTIVITY_SUBTITLE_EXPRESSION =
      "#{instance.registration.name} with id #{instance.id} has de-registered from Spring Boot"
          + " Admin";

  private static final String DEFAULT_REGISTER_ACTIVITY_SUBTITLE_EXPRESSION =
      "#{instance.registration.name} with id #{instance.id} has registered with Spring Boot Admin";

  private static final String DEFAULT_STATUS_ACTIVITY_SUBTITLE_EXPRESSION =
      "#{instance.registration.name} with id #{instance.id} changed status from #{lastStatus} to"
  ...
  /**
   * Theme Color is the color of the accent on the message that appears in Microsoft Teams. Default
   * is Spring Green
   */
  ...
  /**
   * Message will be used as title of the Activity section of the Teams message when an app changes
   * status
   */
  ...
  public MicrosoftTeamsNotifier(InstanceRepository repository, RestTemplate restTemplate) {
    ...
    this.themeColor =
        parser.parseExpression(DEFAULT_THEME_COLOR_EXPRESSION, ParserContext.TEMPLATE_EXPRESSION);
    this.deregisterActivitySubtitle =
        parser.parseExpression(
            DEFAULT_DEREGISTER_ACTIVITY_SUBTITLE_EXPRESSION, ParserContext.TEMPLATE_EXPRESSION);
    this.registerActivitySubtitle =
        parser.parseExpression(
            DEFAULT_REGISTER_ACTIVITY_SUBTITLE_EXPRESSION, ParserContext.TEMPLATE_EXPRESSION);
    this.statusActivitySubtitle =
        parser.parseExpression(
            DEFAULT_STATUS_ACTIVITY_SUBTITLE_EXPRESSION, ParserContext.TEMPLATE_EXPRESSION);
  }
    ...
    return Mono.fromRunnable(
        () ->
            this.restTemplate.postForEntity(
                webhookUrl, new HttpEntity<Object>(message, headers), Void.class));
  }
  ...
  public void setDeregisterActivitySubtitle(String deregisterActivitySubtitle) {
    this.deregisterActivitySubtitle =
        parser.parseExpression(deregisterActivitySubtitle, ParserContext.TEMPLATE_EXPRESSION);
  }
  ...
  public void setRegisterActivitySubtitle(String registerActivitySubtitle) {
    this.registerActivitySubtitle =
        parser.parseExpression(registerActivitySubtitle, ParserContext.TEMPLATE_EXPRESSION);
  }
  ...
  public void setStatusActivitySubtitle(String statusActivitySubtitle) {
    this.statusActivitySubtitle =
        parser.parseExpression(statusActivitySubtitle, ParserContext.TEMPLATE_EXPRESSION);
  }
  ...
  public static class Message {
    ...
    @Builder.Default private final List<Section> sections = new ArrayList<>();
  }
  ...
  public static class Section {
    ...
    @Builder.Default private final List<Fact> facts = new ArrayList<>();
  }
