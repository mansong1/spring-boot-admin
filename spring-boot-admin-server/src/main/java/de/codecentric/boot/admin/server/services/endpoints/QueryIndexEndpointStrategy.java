...
public class QueryIndexEndpointStrategy implements EndpointDetectionStrategy {
  ...
  private static final MediaType actuatorMediaType =
  ...
  protected Function<ClientResponse, Mono<Endpoints>> convert(
      Instance instance, String managementUrl) {
    return (response) -> {
      ...
      log.debug(
          "Querying actuator-index for instance {} on '{}' successful.",
          instance.getId(),
          managementUrl);
      return response
  ...
  protected Function<Endpoints, Endpoints> alignWithManagementUrl(
      InstanceId instanceId, String managementUrl) {
    return (endpoints) -> {
      ...
      log.warn(
          "Endpoints for instance {} queried from {} are falsely using http. Rewritten to https."
              + " Consider configuring this instance to use"
  ...
  protected Mono<Endpoints> convertResponse(Response response) {
    List<Endpoint> endpoints =
        response.getLinks().entrySet().stream()
