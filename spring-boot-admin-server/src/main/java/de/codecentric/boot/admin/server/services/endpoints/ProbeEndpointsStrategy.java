...
public class ProbeEndpointsStrategy implements EndpointDetectionStrategy {
  ...
  public ProbeEndpointsStrategy(InstanceWebClient instanceWebClient, String[] endpoints) {
    ...
    this.endpoints =
        Arrays.stream(endpoints).map(EndpointDefinition::create).collect(Collectors.toList());
  ...
  public Mono<Endpoints> detectEndpoints(Instance instance) {
    if (instance.getRegistration().getManagementUrl() == null) {
      log.debug(
          "Endpoint probe for instance {} omitted. No management-url registered.",
          instance.getId());
      return Mono.empty();
    }
  ...
  protected Mono<DetectedEndpoint> detectEndpoint(Instance instance, EndpointDefinition endpoint) {
    URI uri =
        UriComponentsBuilder.fromUriString(instance.getRegistration().getManagementUrl())
            .path("/")
            .path(endpoint.getPath())
            .build()
            .toUri();
    return this.instanceWebClient
        .instance(instance)
        .options()
        .uri(uri)
  ...
  protected Function<ClientResponse, Mono<DetectedEndpoint>> convert(
      InstanceId instanceId, EndpointDefinition endpointDefinition, URI uri) {
    return (response) -> {
  ...
  protected Mono<Endpoints> convert(List<DetectedEndpoint> endpoints) {
    ...
    Map<String, List<DetectedEndpoint>> endpointsById =
        endpoints.stream().collect(groupingBy((e) -> e.getDefinition().getId()));
    List<Endpoint> result =
        endpointsById.values().stream()
            .map(
                (endpointList) -> {
    ...
    private static EndpointDefinition create(String idWithPath) {
      ...
      if (idxDelimiter < 0) {
        ...
        return new EndpointDefinition(
            idWithPath.substring(0, idxDelimiter), idWithPath.substring(idxDelimiter + 1));
      }
    }
  }
}
